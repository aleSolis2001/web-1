...
for (item of laptops){
    ...
    buttom.classList.add("buy-buttom");
    buttom.innerHTML = "Comprar"

    divProducto.appendChild(img);
    divProducto.appendChild(nombre);
    divProducto.appendChild(precio);
    divProducto.appendChild(buttom)
...
const scrollLeftBtn = document.querySelectorAll(".l-btn");
const scrollRightBtn = document.querySelectorAll(".r-btn");

scrollLeftBtn.forEach(element => {
    element.addEventListener("click", (e) => {
        let widthProduct = document.querySelectorAll(".producto")[0].offsetWidth
        e.stopPropagation()
        element.previousElementSibling.scrollBy({ left: widthProduct - (widthProduct * 2), behavior: "smooth" });

    }); 
});

scrollRightBtn.forEach(element=>{
    element.addEventListener("click", (e) => {
    let widthProduct = document.querySelectorAll(".producto")[0].offsetWidth
    e.stopPropagation()
    let container = element.previousElementSibling.previousElementSibling
    container.scrollBy({ left: widthProduct, behavior: "smooth" });
});
})